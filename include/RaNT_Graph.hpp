...
class RaNT_Graph {
  ...
  public:
    ...
    RaNT_Graph(ygm::comm &comm) : 
                  ...
                  // m_weighted(weighted),
                  // m_directed(directed) 
                  delegation_threshold(comm.size())
                  {
    ...
    void async_start_path(VertexID v, uint32_t l) { 
    ...
    void take_n_paths(uint64_t n, uint32_t k) {
      ...
      for (int p = 0; p < ranks_total_paths; p++) {
        ...
        async_start_path(vertex, path_length);  
      }
    ...
    void read_edges_from_container(Container &cont) {
      ...
      m_adj_lists.for_all([&total_edges, &vertices_to_delegate](const auto& v, auto& adj_list) {
        ...
        if (adj_list.size() >= pthis->delegation_threshold) {
    ...
    struct async_step {            
      ...
      void operator()(uint64_t vertex,
        ...
        pthis->m_comm.cout("Made it inside step functor on rank ", pthis->m_comm.rank());
        path.insert(vertex);
        pthis->m_cs.async_insert(vertex);
        // find new unvisited neighbor to visit next
        if (path.size() < l) {
          if (adj_list.size() > pthis->rejection_threshold) {
    ...
    struct async_delegated_step {            
      ...
      void operator()(uint64_t vertex,
        ...
        pthis->m_comm.cout("Made it inside delegated step functor on rank ", pthis->m_comm.rank());
        path.insert(vertex);
        if (path.size() < l) { 
